/**
 * Auto-generated RPC Stub for MyFunction
 *
 ...
 * @module my-function/stub
 */

import type { GreetResponse, MathResult, MyFunctionTargetMethods, RpcRequest, RpcResponse } from './types'
 ...
 */
export class MyFunctionStubError extends Error {
  readonly code: string
  readonly requestId?: string

  constructor(message: string, code: string, requestId?: string) {
    super(message)
    this.name = 'MyFunctionStubError'
    this.code = code
...
/**
 * RPC Stub client for MyFunction
 *
 ...
 * const stub = new MyFunctionStub(env.MY_SERVICE)
 * const result = await stub.greet('World')
 * console.log(result.message) // "Hello, World!"
 *
 * // Using URL
 * const stub = MyFunctionStub.fromUrl('https://my-function.workers.dev')
 ...
 * ```
 */
export class MyFunctionStub implements MyFunctionTargetMethods {
   ...
   */
  static fromUrl(url: string, retryConfig?: Partial<RetryConfig>): MyFunctionStub {
    return new MyFunctionStub(url, retryConfig)
  }

  /**
   * Create a stub from a service binding
   */
  static fromBinding(binding: Fetcher, retryConfig?: Partial<RetryConfig>): MyFunctionStub {
    return new MyFunctionStub(binding, retryConfig)
  }
  ...
  private async _call<T>(method: string, params: unknown[]): Promise<T> {
    ...
    if ('error' in result) {
      this._errorCount++
      throw new MyFunctionStubError(result.error, result.code, result.id)
    }
  ...
  private async _callWithRetry<T>(method: string, params: unknown[]): Promise<T> {
    ...
    for (let attempt = 0; attempt < this._retryConfig.maxAttempts; attempt++) {
      try {
        ...
        if (error instanceof MyFunctionStubError && (error.isInvalidParams || error.isMethodNotFound)) {
          throw error
        }
  ...
  async greet(name: string): Promise<GreetResponse> {
    if (name === undefined || name === null) {
      throw new MyFunctionStubError('Parameter "name" is required', 'INVALID_PARAMS')
    }
  ...
  async add(a: number, b: number): Promise<MathResult> {
    if (a === undefined || a === null) {
      throw new MyFunctionStubError('Parameter "a" is required', 'INVALID_PARAMS')
    }
    if (b === undefined || b === null) {
      throw new MyFunctionStubError('Parameter "b" is required', 'INVALID_PARAMS')
    }
...
export interface BatchRequest {
  method: keyof MyFunctionTargetMethods
  params: unknown[]
}
 ...
 * ```
 */
export async function batchCall<T>(stub: MyFunctionStub, requests: BatchRequest[]): Promise<BatchResult<T>[]> {
  const promises = requests.map(async (req): Promise<BatchResult<T>> => {
    try {
      ...
      if (error instanceof MyFunctionStubError) {
...
// Export types
export type { GreetResponse, MathResult, MyFunctionTargetMethods }
